...
//        super.configure(http);    // 기존 시큐리티 기능 비활성화
        http.csrf().disable();
        http.authorizeRequests()
                ...
                .and()  // 권한이 없는 페이지에 요청이 오면 로그인 페이지로 이동하도록 지정
                .formLogin()
                .loginPage("/loginForm")
                // login 주소가 호출되면 시큐리티가 낙아채서 대신 로그인 진행 따라서 controller의 /login을 만들 필요가 없음 security가 대신 만들어주니까
